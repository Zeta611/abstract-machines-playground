...
const bool_top = join$5(true_, false_);

function lift_int_binop(topOpt, f, param, param$1) {
  const top = topOpt !== undefined ? topOpt : [
      /* Top */ 0,
      empty$3
    ];
  const y = param$1[0];
...
function M$1(P) {
      ...
      case "add" :
        ...
        if (match) {
          ...
          if (match$1 && !match$1.tl) {
            return Stdlib__Result.ok(lift_int_binop(undefined, (function (n, m) {
              return of_int(n + m | 0);
            }), match.hd, match$1.hd));
          }
      ...
      case "eq" :
        ...
        if (match$2) {
          ...
          if (match$3 && !match$3.tl) {
            return Stdlib__Result.ok(lift_int_binop(bool_top, (function (n, m) {
              if (n === m) {
                return true_;
              } else {
                return false_;
  ...
  const abs_transition = function (sigma) {
    ...
    const l = sigma[1];
    const t = sigma[0];
    const cmd = Curry._2(Amp_lib_s__Ast.LabelMap.find, l, prog.ctrl);
    const x = cmd.desc;
    switch (x.TAG) {
      case /* Return */ 0 :
        ...
        return Stdlib__Result.ok(Stdlib__List.concat_map((function (ptn_ak) {
          return Stdlib__List.map((function (param) {
            const match = param[1];
            const match$1 = Stdlib__Option.get(let_call_of(Curry._2(Amp_lib_s__Ast.LabelMap.find, param[0][1], prog.ctrl)));
            return [
              undefined,
              match$1[1].label,
      ...
      case /* LetCall */ 2 :
        ...
        return Stdlib__Result.Syntax.let$star(Stdlib__Option.to_result("undefined function " + callee, Curry._2(Amp_lib_s__Utils.StringMap.find_opt, callee, prog.defs)), (function (def) {
      ...
      case /* LetTag */ 3 :
        const body$1 = x.body;
        const argVals = Stdlib__List.map((function (arg) {
          return joined_lookup(lookup$2(arg, rho), sv);
        }), x.args);
        ...
        const v_1 = of_tag_args(x.tag, Stdlib__List.map((function (param) {
        ...
        return Stdlib__Result.ok({
          hd: [
            undefined,
            body$1.label,
            weak_update$2(x.x, Curry._1(singleton, a0), rho),
