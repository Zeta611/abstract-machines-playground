...
module type PARAM = sig
  ...
  val ptn_of_label : Label.t -> label_ptn
  val labels_of_ptn : label_ptn -> Label.t array
  val prog : program
end
...
module M (P : PARAM) = struct
  ...
  module AbsFrameKey = struct
    type t = Time.Ptn.t * Label.t

    let compare = compare
  end

  module AbsFrames = PMap (AbsFrameKey) (AbsFrame)
  module AbsKStore = PMap (KAddr.Ptn) (AbsFrames)
  ...
  let abs_tick (_ : abs_state)
      (_ : [ `TL of Time.Ptn.t * Label.t | `L of Label.t ]) : Time.Ptn.t =
    ()
  ...
  let abs_transition ((t, l, rho, sv, sk, ak) as sigma : abs_state) :
  ...
  module AbsCfg = struct
    module TimeLabelPtn = struct
      type t = Time.Ptn.t * label_ptn

      let compare = compare
    end

    module Frames = PMap (TimeLabelPtn) (AbsFrame)
    include Pair (Frames) (Pair (AbsVStore) (AbsKStore))
  end

  let abs_transfer (cfg : AbsCfg.t) : (AbsCfg.t, string) result =
    let map, (sv, sk) = cfg in
    let+ newCfgs =
      AbsCfg.Frames.to_list map
      |> List.concat_map (fun ((t, l), (rho, ak)) ->
          labels_of_ptn l
          |> Array.map (fun lbl ->
              let sigma = (t, lbl, rho, sv, sk, ak) in
              abs_transition sigma)
          |> Array.to_list)
      |> seq
    in
    ...
    |> List.fold_left
         (fun cfg ((t, l, rho, sv, sk, ak) : abs_state) ->
           AbsCfg.join cfg
             ( AbsCfg.Frames.weak_update
                 (t, ptn_of_label l)
                 (rho, ak) AbsCfg.Frames.bot,
               (sv, sk) ))
         cfg
  ...
  let show_label (Label.L n) = Printf.sprintf "L%d" n

  let show_label_ptn l =
    labels_of_ptn l |> Array.map show_label |> Array.to_list
    |> String.concat "|"
  ...
  let view_frame (((t, l), (rho, ak)) : AbsCfg.TimeLabelPtn.t * AbsFrame.t) :
      abs_frame_row =
    {
      time = show_time t;
      label_ptn = show_label_ptn l;
      env = view_env rho;
      kont = show_kaddr_abs ak;
    }

  let view_kstore_row ((addr, frames) : KAddr.Ptn.t * AbsFrames.t) :
      abs_kstore_row list =
    AbsFrames.to_list frames
    |> List.map (fun ((time, label), (rho, ak)) ->
        {
  ...
  let view_cfg ((frames, (sv, sk)) : AbsCfg.t) : abs_cfg_view =
    {
      frames =
        AbsCfg.Frames.to_list frames |> List.map view_frame |> Array.of_list;
      vstore = AbsVStore.to_list sv |> List.map view_vstore_row |> Array.of_list;
      kstore =
        AbsKStore.to_list sk |> List.concat_map view_kstore_row |> Array.of_list;
    }

  let abs_inject ((rho, sv) : AbsEnv.t * AbsVStore.t) : AbsCfg.t =
    let main = StringMap.find prog.mainName prog.defs in
    ( AbsCfg.Frames.weak_update
        ((), ptn_of_label main.body.label)
        (rho, KAddr.Abs.bot) AbsCfg.Frames.bot,
      (sv, AbsKStore.bot) )

  let rec iterate_abs_transfer (fuel : int) (steps : int) (cfg : AbsCfg.t) :
      (abs_run, string) result =
    if fuel <= 0 then Ok { cfg; steps; stabilized = false }
    else
      let* next = abs_transfer cfg in
      if next = cfg then Ok { cfg = next; steps = steps + 1; stabilized = true }
      else iterate_abs_transfer (fuel - 1) (steps + 1) next

  let run_abs ((rho, sv) : AbsEnv.t * AbsVStore.t) (fuel : int) :
      (abs_run, string) result =
    abs_inject (rho, sv) |> iterate_abs_transfer fuel 0
end
